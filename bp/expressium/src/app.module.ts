 ...
 * @param useRateLimiter - Whether to enable rate limiting middleware (default: false).
 * 
 * @returns Configured Express application.
 * 
 * @throws If access log creation fails.
 */
const configureApp = async (app: Express, useRateLimiter?: boolean): Promise<Application> => {
 ...
 */
export const createServer = async (app: Express): Promise<Express.Application> => {
  const [configuredApp] = await Promise.all(
    [
      configureApp(app),
      //loadSSLCertificates()
    ]
