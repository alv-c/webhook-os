 ...
 * 
 * @description The BasicAndTokenStrategy class provides a sophisticated authentication 
...
export class BasicAndTokenStrategy implements IAuthenticationStrategy.IAuthenticationStrategy {
   ...
   * @param username - Username for basic authentication to the token endpoint.
   * @param password - Password for basic authentication to the token endpoint.
   * @param query - Optional query parameters to send with the authentication request.
   * @param headers - Optional headers to send with the authentication request.
   ...
   * @param expirationExtractor - Custom function to extract expiration time from authentication response. Defaults to 0.
   * @param expirationBuffer - Time in milliseconds to renew token before actual expiration (default 0). Creates a safety margin.
   * 
  ...
  public constructor(
     ...
     * @readonly
     */
    private readonly tokenExtractor: (response: AxiosResponse) => string = (response: AxiosResponse): string => response.data.data.token,
    
    /**
     * @private
     * @readonly
     */
    private readonly expirationExtractor: (response: AxiosResponse) => number = (): number => 0,

    /**
     * @private
     * @readonly
     */
    private readonly expirationBuffer: number = 0
  ) {}
   ...
   * 
   * @description 
   * Determines token validity by checking if:
   * 
   * - Token exists
   ...
   * 
   * @description 
   * Ensures a valid token is available by:
   * 
